...
import { CartItem, Cart as CartEntity, Coupon } from '../../shared/models/cart';
...
export class Cart {
  ...
  totals = computed(() => {
    ...
    const discount = cart.coupon
      ? this.roundCurrency(
          cart.coupon.amountOff ?? (subtotal * (cart.coupon.percentOff ?? 0) / 100)
        )
      : 0;
    return {
      subtotal, 
      shipping,
      discount,
      total: this.roundCurrency(subtotal + shipping - discount)
    }
  });
  ...
  setCart(cart: CartEntity) {
    return this.http.post<CartEntity>(this.baseUrl + 'cart', cart).pipe(
      tap(cart => {
        this.cart.set(cart)
      })
    );
